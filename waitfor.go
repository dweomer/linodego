...
func (client Client) NewEventPollerWithSecondary(
	ctx context.Context, id int, primaryEntityType EntityType, secondaryID string, action EventAction,
) (*EventPoller, error) {
...
func (p *EventPoller) WaitForLatestUnknownEvent(ctx context.Context) (*Event, error) {
	...
	for {
		...
		case <-ticker.C:
			...
			for _, event := range events {
				if p.SecondaryEntityID != "" && !eventMatchesSecondary(p.SecondaryEntityID, event) {
					continue
				}
...
func eventMatchesSecondary(configuredID string, e Event) bool {
